...
function Dropdown({ label, items, footer }: { label: string; items: DropdownItem[]; footer?: { label: string; href: string } }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="relative" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
      <button className="flex items-center gap-1.5 text-[var(--muted)] hover:text-[var(--off-white)] transition-colors text-sm" style={{ fontFamily: "var(--font-inter)" }}>
        {label}
        <svg className={`w-3 h-3 transition-transform duration-200 ${open ? "rotate-180" : ""}`} fill="none" viewBox="0 0 12 12">
          <path d="M2 4l4 4 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </button>

      <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-3 border border-[var(--border)] rounded-sm overflow-hidden shadow-2xl transition-all duration-200 ${open ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-1 pointer-events-none"}`}
        style={{ background: "rgba(18,20,28,0.97)", backdropFilter: "blur(16px)", minWidth: 200 }}>
        {items.map((item, i) => (
          <Link key={item.href} href={item.href}
            className="flex items-start justify-between px-4 py-3 hover:bg-white/[0.04] transition-colors group"
            style={{ borderBottom: i < items.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}
            onClick={() => setOpen(false)}>
            <span className="text-sm text-[var(--muted)] group-hover:text-[var(--off-white)] transition-colors" style={{ fontFamily: "var(--font-inter)" }}>
              {item.label}
            </span>
            {item.sub && (
              <span className="text-[10px] text-[var(--muted)] ml-3 mt-0.5 shrink-0 opacity-60" style={{ fontFamily: "var(--font-inter)" }}>{item.sub}</span>
            )}
          </Link>
        ))}
        {footer && (
          <Link href={footer.href}
            className="flex items-center gap-1 px-4 py-3 text-xs font-medium text-[var(--coral)] hover:bg-white/[0.04] transition-colors"
...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)", background: "rgba(14,15,20,0.88)", backdropFilter: "blur(20px)" }}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <a
          ...
          className="hidden md:inline-flex items-center gap-2 text-sm font-medium text-white transition-all hover:translate-y-[-1px]"
          style={{
            fontFamily: "var(--font-inter)",
            background: "var(--coral)",
            padding: "8px 18px",
            borderRadius: 4,
            boxShadow: "0 0 20px rgba(77,128,255,0.2)",
          }}
        >
          Book a Call
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ${open ? "max-h-screen" : "max-h-0"}`}
        style={{ borderTop: open ? "1px solid rgba(255,255,255,0.06)" : "none", background: "rgba(14,15,20,0.97)" }}>
        <div className="px-6 py-5 flex flex-col gap-1 text-sm" style={{ fontFamily: "var(--font-inter)" }}>
          ...
          {solutionsNav.map(s => (
            <Link key={s.href} href={s.href} onClick={() => setOpen(false)} className="py-2 pl-3 text-[var(--muted)] hover:text-[var(--off-white)] transition-colors flex items-center justify-between">
              <span>{s.label}</span>
          ...
          {industries.map(ind => (
            <Link key={ind.href} href={ind.href} onClick={() => setOpen(false)} className="py-2 pl-3 text-[var(--muted)] hover:text-[var(--off-white)] transition-colors">
              {ind.label}
            </Link>
          ))}
          ...
          {resources.map(r => (
            <Link key={r.href} href={r.href} onClick={() => setOpen(false)} className="py-2 pl-3 text-[var(--muted)] hover:text-[var(--off-white)] transition-colors">
              {r.label}
            </Link>
          ))}
          ...
          <a href="https://cal.com/ali-z.s-yb9uld/data-strategy-assessment" target="_blank" rel="noopener noreferrer"
            className="mt-3 py-3 text-center text-white font-medium rounded-sm"
