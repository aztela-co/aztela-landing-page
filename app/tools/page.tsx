...
function calcDistributor(revenue: number, locations: number, skus: number) {
  ...
  const total            = stockoutLoss + expediteCost + deadStockCost + reconTime;
  return [
...
function calcWholesaler(revenue: number, suppliers: number, quotes: number) {
  ...
  const total            = marginLeakage + deadStock + pricingLag + reconTime;
  return [
...
export default function ToolsPage() {
  ...
  const field2Label: Record<Segment, string> = { distributor: "Number of locations / branches", wholesaler: "Number of active suppliers", manufacturer: "Number of production lines" };
  const field3Label: Record<Segment, string> = { distributor: "Active SKU count (approx)", wholesaler: "Open quotes at any time", manufacturer: "Monthly line stoppages from component shortages" };
      ...
      <main className="pt-16">
        ...
        <section className="px-6 py-16 md:py-24 grid-bg">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-[var(--coral)] text-xs font-medium tracking-[0.2em] uppercase mb-4" style={{ fontFamily: "var(--font-inter)" }}>Operational ROI Calculator</p>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex gap-1 p-1 border border-[var(--border)] rounded-sm w-fit mb-10">
              {(["distributor", "wholesaler", "manufacturer"] as Segment[]).map((s) => (
                <button key={s} onClick={() => { setSegment(s); setCalculated(false); if (s === "manufacturer") { setField3(3); setField4(100); } else { setField3(50); } }}
            ...
            <div className="grid md:grid-cols-2 gap-10 items-start">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <input type="range" min={5} max={250} value={revenue} onChange={e => { setRevenue(+e.target.value); setCalculated(false); }}
                ...
                <div>
                  ...
                  <input type="range" min={1}
                    max={segment === "manufacturer" ? 15 : segment === "wholesaler" ? 200 : 20000}
                    value={field3}
                    onChange={e => { setField3(+e.target.value); setCalculated(false); }}
                    className="w-full h-1 rounded-full appearance-none cursor-pointer"
                    style={{ background: `linear-gradient(to right, var(--coral) ${segment === "manufacturer" ? (field3 - 1) / 14 * 100 : segment === "wholesaler" ? (field3 - 1) / 199 * 100 : (field3 - 1) / 19999 * 100}%, var(--border) 0%)` }} />
                  <div className="flex justify-between text-xs text-[var(--muted)] mt-1" style={{ fontFamily: "var(--font-inter)" }}>
                    {segment === "manufacturer" ? <><span>1</span><span>15</span></> : segment === "wholesaler" ? <><span>1</span><span>200</span></> : <><span>1K</span><span>20K</span></>}
                  </div>
                </div>
